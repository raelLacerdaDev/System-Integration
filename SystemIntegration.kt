...
fun csvToJson (videos: Array<String>): String {
    val processingPatternDigits = """\d{2}-\d{2}""".toRegex()
    val processingPatternMp4 = """\.mp4""".toRegex()
    val parsedVideos = videos.map { videos ->
        val parts = videos.split(",")
        Video(
            id = parts[0],
            title = parts[1]
                .replace(processingPatternDigits,"")
                .replace(processingPatternMp4,"")
                .trimIndent(),
...
private data class Video (val id: String, val title: String, val duration: Long) {
